...
(function(){
  ...
  function injectStyle(){
    ...
    style.type = 'text/css';
    style.textContent = '\n/* Table + button styles only - modal visuals rely on theme.css promo-modal rules */\n.tg{border-collapse:collapse;border-spacing:0}.tg td{border-color:black;border-style:solid;border-width:1px;font-family:Arial, sans-serif;font-size:14px;overflow:hidden;padding:10px 5px;word-break:normal}.tg th{border-color:black;border-style:solid;border-width:1px;font-family:Arial, sans-serif;font-size:14px;font-weight:700;overflow:hidden;padding:10px 5px;word-break:normal}.tg .tg-0lax{text-align:left;vertical-align:middle}\n.size-guide-button{display:inline-block;margin:4px 0 8px 0;padding:4px 8px;background:#111;color:#fff;border-radius:3px;text-decoration:none;cursor:pointer;font-size:12px;line-height:1;border:none}\n/* Modal title/subtitle */\n.size-guide-modal-title{font-size:18px;font-weight:700;text-align:center;margin:10px 0 4px;color:#111}\n.size-guide-modal-subtitle{font-size:12px;color:#666;text-align:center;margin:0 0 12px}\n.promo-modal .tg{width:100%;max-width:520px;margin:12px auto;border-collapse:collapse}\n@media(max-width:520px){.promo-modal .tg{margin:8px auto}}\n';
  ...
  function buildTable(rows){
    ...
    // Calculate rowspan for first column (merge cells with same value)
    var rowspans = {}; // row index -> {colIndex: spanCount}
    for (var col = 0; col < 1; col++) { // Apply to first column
      var row = 1;
      while (row < rows.length) {
        var currentValue = rows[row][col];
        var span = 1;
        while (row + span < rows.length && rows[row + span][col] === currentValue) {
          span++;
        }
        if (span > 1) {
          if (!rowspans[row]) rowspans[row] = {};
    ...
    var skipCells = {}; // "row,col" format to track cells that are part of a rowspan
    for (var i=1; i<rows.length; i++){ 
      var tr = document.createElement('tr'); 
      for (var colIdx = 0; colIdx < rows[i].length; colIdx++) {
        var cellKey = i + ',' + colIdx;
        if (skipCells[cellKey]) continue; // skip cells covered by rowspan
        
        var td = document.createElement('td'); 
        td.className='tg-0lax'; 
        td.innerText = rows[i][colIdx];
        
        // Apply rowspan if this cell spans multiple rows
        if (rowspans[i] && rowspans[i][colIdx]) {
          var span = rowspans[i][colIdx];
          td.setAttribute('rowspan', span);
          // Mark future rows as skipped for this column
          for (var s = 1; s < span; s++) {
            skipCells[(i+s) + ',' + colIdx] = true;
          }
        }
  ...
  // Title and subtitle for the modal
  var titleEl = document.createElement('div'); titleEl.className = 'size-guide-modal-title'; titleEl.innerText = 'SIZE GUIDE';
  var subtitleEl = document.createElement('div'); subtitleEl.className = 'size-guide-modal-subtitle'; subtitleEl.innerText = 'all measurements are labeled in inches';
  var footnoteEl = document.createElement('div'); footnoteEl.style.cssText = 'font-size:11px;color:#999;text-align:center;margin:10px 0 4px'; footnoteEl.innerText = 'Product measurements may vary by up to 2"';
  modalContent.appendChild(closeLabel);
  modalContent.appendChild(titleEl);
  modalContent.appendChild(subtitleEl);
  modalContent.appendChild(table);
  modalContent.appendChild(footnoteEl);
