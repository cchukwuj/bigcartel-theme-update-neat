...
(function(){
  ...
  // create a label as the close control (matches promo modal implementation)
  ...
  var lastFocus = null;
  function openModal(){ lastFocus = document.activeElement; modal.style.opacity = '1'; modal.style.visibility = 'visible'; modal.style.pointerEvents = 'auto'; modal.setAttribute('aria-hidden','false'); btn.setAttribute('aria-expanded','true'); try{ document.body.classList.add('no-scroll'); }catch(e){}; setTimeout(function(){ modalContent.focus(); }, 10); }
  function closeModal(){ lastFocus = lastFocus || null; modal.style.opacity = ''; modal.style.visibility = ''; modal.style.pointerEvents = ''; modal.setAttribute('aria-hidden','true'); btn.setAttribute('aria-expanded','false'); try{ document.body.classList.remove('no-scroll'); }catch(e){}; if (lastFocus && lastFocus.focus) { lastFocus.focus(); } }
  ...
  // also add a direct click handler on the label to ensure close happens
  closeLabel.addEventListener('click', function(){ if (modalToggle.checked){ modalToggle.checked = false; modalToggle.dispatchEvent(new Event('change')); } }, { passive: true });
  // overlay click closes by unchecking
  modalOverlayInner.addEventListener('click', function(){ modalToggle.checked = false; modalToggle.dispatchEvent(new Event('change')); }, { passive: true });
  // Esc closes
