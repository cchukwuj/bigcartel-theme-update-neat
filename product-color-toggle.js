...
(function() {
  ...
  /**
   * Get the color option group/select element
   */
  function getColorOptionElement() {
    // First try to find option group (for Big Cartel's multi-option setup)
    const optionGroups = document.querySelectorAll('.product_option_group');
    ...
    // Fallback: look for select with "color" in any associated labels
    const selects = document.querySelectorAll('select');
    for (let select of selects) {
      const parentLabel = select.closest('.select')?.parentElement?.innerText || '';
      if (parentLabel.toLowerCase().includes('color')) {
        return select;
      }
    }

    // If only one select exists, assume it's the color
    if (selects.length === 1) {
      return selects[0];
    }
  ...
  function createToggleSwitch(isOn = false) {
    ...
    container.className = 'color-toggle-container';
    container.innerHTML = `
      <label class="color-toggle-label">Color</label>
      <div class="color-toggle-wrapper">
        <input type="checkbox" id="colorToggle" class="color-toggle-input" ${isOn ? 'checked' : ''}>
        <label for="colorToggle" class="color-toggle-switch">
  ...
  function addToggleStyles() {
    if (document.getElementById('color-toggle-styles')) return;

    const style = document.createElement('style');
    style.id = 'color-toggle-styles';
    style.textContent = `
      ...
      .color-toggle-input:checked + .color-toggle-switch .toggle-slider {
        background-color: #333;
      }

      .color-toggle-input:checked + .color-toggle-switch .toggle-slider::before {
      ...
      .color-toggle-input:checked + .color-toggle-switch .toggle-text-off {
        display: none;
      }

      .color-toggle-input:checked + .color-toggle-switch .toggle-text-on {
        display: inline;
      }
  ...
  function updateProductOption(isToggleOn) {
    const colorElement = getColorOptionElement();
    ...
    const selectElement = colorElement.tagName === 'SELECT' 
      ? colorElement 
      : colorElement.querySelector('select');
  ...
  function initToggle() {
    ...
    if (!toggle) return;
    colorElement.parentElement?.insertBefore(toggle, colorElement);
