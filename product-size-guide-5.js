...
(function(){
  ...
  var closeModalBtn = document.createElement('button'); closeModalBtn.type = 'button'; closeModalBtn.className = 'promo-modal-close'; closeModalBtn.setAttribute('aria-label', 'Close size guide'); closeModalBtn.innerText = '×';
  ...
  function closeModal(){ modal.style.opacity = ''; modal.style.visibility = ''; modal.style.pointerEvents = ''; modal.setAttribute('aria-hidden','true'); btn.setAttribute('aria-expanded','false'); try{ document.body.classList.remove('no-scroll'); }catch(e){}; if (lastFocus && lastFocus.focus) { lastFocus.focus(); } }
  ...
  modalOverlayInner.addEventListener('click', function(){ closeModal(); }, { passive: true });
  document.addEventListener('keydown', function(e){ if ((e.key === 'Escape' || e.key === 'Esc') && modal.style.pointerEvents === 'auto'){ closeModal(); } });
